...
public class SigfoxProtocolDecoder extends BaseHttpProtocolDecoder {
    ...
    protected Object decode(
        ...
        if (jsonContains(json, "location")
            ...
            position.setLatitude_wgs84(getJsonDouble(location, jsonContains(location, "lat") ? "lat" : "latitude"));
            position.setLongitude_wgs84(getJsonDouble(location, jsonContains(location, "lng") ? "lng" : "longitude"));
            ...
            try {
                ...
                if ("Amber".equals(getDeviceModel(deviceSession))) {
                    ...
                    position.setValid(true);
                    position.setLatitude_wgs84(buf.readInt() / 60000.0);
                    position.setLongitude_wgs84(buf.readInt() / 60000.0);

                } else if (header == 0x0f || header == 0x1f) {

                    position.setValid(header >> 4 > 0);
                    position.setLatitude_wgs84(BufferUtil.readSignedMagnitudeInt(buf) * 0.000001);
                    position.setLongitude_wgs84(BufferUtil.readSignedMagnitudeInt(buf) * 0.000001);
            ...
            try {
                ...
                while (buf.isReadable()) {
                    ...
                    switch (type) {
                        case 0x01 -> {
                            position.setValid(true);
                            position.setLatitude_wgs84(buf.readMedium());
                            position.setLongitude_wgs84(buf.readMedium());
                        }
                        case 0x02 -> {
                            position.setValid(true);
