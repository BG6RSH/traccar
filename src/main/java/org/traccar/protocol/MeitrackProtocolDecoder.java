...
public class MeitrackProtocolDecoder extends BaseProtocolDecoder {
    ...
    private List<Position> decodeBinaryC(Channel channel, SocketAddress remoteAddress, ByteBuf buf) {
        ...
        while (buf.readableBytes() >= 0x34) {
            ...
            position.setLatitudeWgs84(buf.readIntLE() * 0.000001);
            position.setLongitudeWgs84(buf.readIntLE() * 0.000001);
    ...
    private List<Position> decodeBinaryE(Channel channel, SocketAddress remoteAddress, ByteBuf buf) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            for (int j = 0; j < paramCount; j++) {
                ...
                int id = extension ? buf.readUnsignedShort() : buf.readUnsignedByte();
                switch (id) {
                    case 0x02 -> position.setLatitudeWgs84(buf.readIntLE() * 0.000001);
                    case 0x03 -> position.setLongitudeWgs84(buf.readIntLE() * 0.000001);
