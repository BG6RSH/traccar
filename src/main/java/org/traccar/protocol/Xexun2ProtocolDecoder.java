            ...
            for (int i = 0; i < count; i++) {
                ...
                if (BitUtil.check(mask, 1)) {
                    ...
                    if (BitUtil.check(positionMask, 0)) {
                        ...
                        position.setLongitude_wgs84(convertCoordinate(buf.readFloat()));
                        position.setLatitude_wgs84(convertCoordinate(buf.readFloat()));
                    }
                    ...
                    if (BitUtil.check(positionMask, 6)) {
                        ...
                        position.setFixTime(position.getDeviceTime());
                        position.set(Position.KEY_SATELLITES, buf.readUnsignedByte());
                        position.setLongitude_wgs84(convertCoordinate(buf.readDouble()));
                        position.setLatitude_wgs84(convertCoordinate(buf.readDouble()));
                    }
